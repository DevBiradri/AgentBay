...
const Chat = () => {
  ...
  const handleSendMessage = async () => {
    ...
    setInputText("");
    resetTranscript(); // clear the mic transcript
    SpeechRecognition.stopListening(); // auto-stop mic
    setIsListening(false);
    setIsLoading(true);
        ...
        <div className="max-w-3xl mx-auto space-y-4 ">
          {messages.length === 0 && (
            <div className="text-center space-y-4 py-12">
              ...
              <p className="text-muted-foreground">
                {mode === "buyer"
                  ? 'Try: "Find me vintage sneakers under $100"'
          ...
          {messages.map((message) => (
            <div key={message.id} className={`flex ${message.isUser ? "justify-end" : "justify-start"}`}>
              <div className={`max-w-xs lg:max-w-md px-4 py-2 rounded-lg ${message.isUser ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground"}`}>
                {message.text}
              </div>
            </div>
          ))}
